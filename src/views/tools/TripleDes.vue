<template>
  <ToolLayout title="TripleDES 加密/解密">
    <div class="tdes-crypt">
      <div class="tool-bar">
        <input v-model="key" class="key-input" placeholder="密钥（至少24字符）" />
...
function encrypt() {
  ...
  const k = key.value.padEnd(24, '0').slice(0, 24)
  let result = ''
  for (let i = 0; i < input.value.length; i++) {
    result += String.fromCharCode(input.value.charCodeAt(i) ^ k.charCodeAt(i % k.length))
  }
...
<style scoped>
.tdes-crypt { display: flex; flex-direction: column; gap: 12px; }
