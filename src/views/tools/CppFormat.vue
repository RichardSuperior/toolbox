    ...
    <div class="tool-wrap">
      <div class="toolbar">
        ...
        <label class="opt-label">缩进: <select v-model="indent" class="select">
          <option value="2">2空格</option><option value="4">4空格</option><option value="	">Tab</option>
        </select></label>
      </div>
      <div class="editor-wrap">
        <div><div class="label">C++ 源代码</div>
          <textarea v-model="input" rows="20" class="code-area" placeholder="粘贴 C++ 代码..."></textarea></div>
...
function format() {
  ...
  const IND = indent.value === '\t' ? '\t' : ' '.repeat(+indent.value)
  let level = 0, result = '', lines = code.split('\n')
