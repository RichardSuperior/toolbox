  ...
  <ToolLayout title="Ruby 代码格式化">
    <div class="tool-wrap">
      ...
      <div class="editor-wrap">
        <div><div class="label">Ruby 源代码</div>
          <textarea v-model="input" rows="20" class="code-area" placeholder="粘贴 Ruby 代码..."></textarea></div>
...
const input = ref(''), output = ref(''), indent = ref('2')
const OPEN = /\b(def|do|class|module|if|unless|while|until|for|begin|case|then)\b/
const CLOSE = /^\s*(end|rescue|ensure|else|elsif|when)\b/
function format() {
  ...
  for (let line of input.value.split('\n')) {
    ...
    if (OPEN.test(t) && !t.endsWith('end')) level++
  }
